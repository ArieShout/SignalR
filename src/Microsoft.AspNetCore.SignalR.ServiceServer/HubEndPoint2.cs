...
{
    public class HubEndPoint2<THub> : HubEndPoint<THub>, IInvocationBinder where THub : Hub
    {
        ...
        public HubEndPoint2(HubLifetimeManager<THub> lifetimeManager,
                           IHubProtocolResolver protocolResolver,
                           IHubContext<THub> hubContext,
                           IOptions<HubOptions> hubOptions,
                           ILogger<HubEndPoint2<THub>> logger,
        ...
        public new async Task OnConnectedAsync(ConnectionContext connection)
        {
        ...
        private async Task RunHubAsync(HubConnectionContext2 connection)
        {
        ...
        private async Task HubOnConnectedAsync(HubConnectionContext2 connection)
        {
        ...
        private async Task HubOnDisconnectedAsync(HubConnectionContext2 connection, Exception exception)
        {
            await Task.CompletedTask;
            //try
            //{
        ...
        private async Task DispatchMessagesAsync(HubConnectionContext2 connection)
        {
        ...
        private async Task ProcessInvocation(HubConnectionContext2 connection,
        ...
        private async Task SendMessageAsync(HubConnectionContext2 connection, HubMessage hubMessage)
        {
        ...
        private async Task Invoke(HubMethodDescriptor descriptor, HubConnectionContext2 connection,
            HubMethodInvocationMessage hubMethodInvocationMessage, bool isStreamedInvocation)
        {
        ...
        private async Task SendInvocationError(HubMethodInvocationMessage hubMethodInvocationMessage,
            HubConnectionContext2 connection, string errorMessage)
        {
        ...
        private void InitializeHub(THub hub, HubConnectionContext2 connection)
        {
        ...
        private async Task StreamResultsAsync(string invocationId, HubConnectionContext2 connection, IAsyncEnumerator<object> enumerator)
        {
            string error = null;
        ...
        private async Task<bool> ValidateInvocationMode(Type resultType, bool isStreamedInvocation,
            HubMethodInvocationMessage hubMethodInvocationMessage, HubConnectionContext2 connection)
        {
        ...
        private IAsyncEnumerator<object> GetStreamingEnumerator(HubConnectionContext2 connection, string invocationId, ObjectMethodExecutor methodExecutor, object result, Type resultType)
        {
            if (result != null)
            {
                var observableInterface = IsIObservable(resultType) ?
